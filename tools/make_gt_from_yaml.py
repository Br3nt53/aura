    ...
    for t_step in range(int(sim_time_s * rate_hz)):
        ...
        for target in scenario_data.get("targets", []):
            ...
            if start_time <= time_s <= end_time:
                ...
                gt_data.append({"frame": frame, "id": target["id"], "x": x, "y": y})
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate ground truth JSONL from a scenario YAML."
    )
    parser.add_argument(
    ...
    if args.test_case:
        if "test_cases" in data and args.test_case in data["test_cases"]:
            ...
            raise ValueError(
                f"Test case '{args.test_case}' not found in {args.scenario}"
            )
    ...
    print(
        f"Successfully generated ground truth for '{args.test_case or 'main scenario'}' to {args.out}"
    )
