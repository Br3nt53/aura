...
def run_single_experiment(scenario_path, params_path, out_dir, test_case=None):
    ...
    4. Saves the metrics.
    """
    print(f"Running experiment for scenario: {scenario_path} with params: {params_path}")
    if test_case:
    ...
    try:
        ...
        gt_df_mot = gt_df.rename(columns={'frame': 'FrameId', 'id': 'Id', 'x': 'X', 'y': 'Y'})
        pred_df_mot = pred_df.rename(columns={'frame': 'FrameId', 'id': 'Id', 'x': 'X', 'y': 'Y'})
        ...
        for frame in sorted(gt_df_mot.FrameId.unique()):
            ...
            distances = mm.distances.norm2squared_matrix(gt_points, pred_points, max_d2=1.0)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run a single AURA experiment and evaluate it.")
    ...
    parser.add_argument('--out-dir', required=True, help="Directory to save the output files (gt, pred, metrics).")
